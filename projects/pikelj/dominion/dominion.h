 ...
 * Date: 2017.07.05
 * Assignment: #2
 * Citation: Code from various sources, baseline from Kristen Bartosz
 * *************************************/
...
/*Here start the user functions that I wrote to refactor the cards */
...
/* plays the smithy card effect on the game
 * allows the player to draw 3 additional cards */

int playCouncilRoom(int currentPlayer, struct gameState *state, int handPos);
/* plays the council room card effect
 * player draws +4 cards, player gets +1 buy, all other players +1 card */
...
/* trashes the feast card and player gets to choose another card up to 5 treasure */

int playGardens();
/* does nothing just returns -1 */

int playEmbargo(int currentPlayer, struct gameState *state, int handPos, int choice1, int *coin_bonus);
/* adds an embargo token to the supply card in play per choice1 */

int playSeaHag(int currentPlayer, struct gameState *state);
/* all other players discard top card from deck, gain a curse and put it on top of deck*/

int playMinion(int currentPlayer, struct gameState *state, int handPos, int choice1, int choice2, int* coin_bonus);
/* +1 action, choice1 is +2 coins, choice2 is redraw*/
