 ...
 * ONID: pikelj
 * Assignment: #3
 * Description: this is a unit test file for the village_effect function in dominion.c
 * *************************************/
 ...
 * Parameters: none
 * Preconditions: none
 * Postconditions: unit test the village_effect function
...
int main(void){
    ...
    for(p = 2; p< 5; p++){
        printStars();
        printf("Unit test for the village_effect function\n");
        testGame = newGame();
        preGame = newGame();
        ...
        validate(village_effect(0, testGame, 5), RETSUCCESS);
