 ...
 *  Preconditions: passed valid player, and gameState
 *  Postconditions: checks if adventurer's memory is correct
 *  Description: see below for detailed description
 *  ***************************************/
...
int checkAdventurerCard(int p, struct gameState *post, int iteration){
    ...
    int drawntreasure_post = 0;
    int cardDrawn = 0;
    int z_post = 0;
    int tempHand_post[MAX_HAND];

    handpos = floor(Random() * MAX_HAND);
    memcpy(&pre, post, sizeof(struct gameState));
    if (adventurer_effect(drawntreasure_post, p, cardDrawn, z_post, tempHand_post, 
                post, handpos) != 0){
 ...
 *  Preconditions: passed valid gameState, and player
 *  Postconditions: adds at least 3 treasure cards
 *  Description: Adds at least 3 treasure cards to the deck and the discard piles of
 *  the gameState passed in and the player passed in
 *  ***************************************/
 ...
 *  Parameters:none
 *  Preconditions: none
 *  Postconditions: adventurer card randomly tested
 ...
 *  of these values in the adventurer function and from the dependencies such as
 *  drawCard and discardCard that are called inside of adventurer.
 *  Adventurer also needs to have at least 2 treasure cards somewhere in the
 *  deck or the discard pile, so use the addTreasure function to add some
 *  randomly chosen locations for treasure cards.
...
int main(void){
    ...
    fprintf(stdout, "Testing Adventurer randomly\n");
