...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
    {
    case adventurer:
     	return adventurer_effect(drawntreasure, currentPlayer, cardDrawn, z, temphand, state, handPos);
    ...
    case mine:
            return mine_effect(state, choice1, choice2, currentPlayer, handPos);
...
int smithy_effect(int currentPlayer, int handPos, struct gameState *state){
    int i;
    //+3 Cards
    for (i = 0; i < 4; i++) //bug, increased boundary from 3 to 4
    {
        drawCard(currentPlayer, state);
    }
