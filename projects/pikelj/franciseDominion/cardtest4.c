 ...
 * ONID: pikelj
 * Assignment: #3
 * Description: this is a unit test file for the minion function in dominion.c
 * *************************************/
 ...
 * Parameters: none
 * Preconditions: none
 * Postconditions: unit test the minion function
...
int main(void){
    ...
    for(m = 0; m < 4; m++){
        printStars();
        printf("Unit test for the minion function\n");
...
/*        validate(minion(0, testGame, 5, choice1, choice2, &coinBonus), RETSUCCESS);*/
        validate(cardEffect(minion, choice1, choice2, -1, testGame, 5, &coinBonus)
                , RETSUCCESS);
